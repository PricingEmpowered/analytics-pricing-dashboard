            ...
            {loading ? (
              Array.from({ length: 8 }, (_, i) => (
                <tr key={i} className="animate-pulse">
                  ...
                  <td className="px-6 py-3"><div className="h-3 w-12 bg-gray-100 rounded ml-auto" /></td>
                  <td className="px-6 py-3"><div className="h-5 w-14 bg-gray-100 rounded ml-auto" /></td>
                </tr>
              ))
            ) : data.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-gray-300 text-sm">No product data available</td>
              </tr>
            ) : (
              data.map((p, i) => (
                <tr key={p.item_number} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-3 text-gray-400 text-xs font-medium">{i + 1}</td>
                  <td className="px-6 py-3">
                    <div>
                      <p className="font-medium text-gray-900 text-sm">{p.item_number}</p>
                      <p className="text-xs text-gray-400 truncate max-w-[180px]">{p.product_cat_description}</p>
                    </div>
                  </td>
                  <td className="px-6 py-3 text-right font-semibold text-gray-800">{fmt(p.revenue)}</td>
                  <td className="px-6 py-3 text-right text-gray-500">{p.qty_shipped.toFixed(0)}</td>
                  <td className="px-6 py-3 text-right"><MarginBadge pct={p.margin_pct} /></td>
                </tr>
              ))
            )}
          </tbody>
